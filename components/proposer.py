...
class DualSidedLLMProposer(LLMPairwiseProposerWithQuestion):
    ...
    def propose(
        ...
        for i in range(self.args["num_rounds"]):
            ...
            hypotheses, logs, questions = self.get_hypotheses(sampled_dataset1, sampled_dataset2)
            all_hypotheses['Model A contains more'].append(hypotheses['Model A contains more'])
            all_hypotheses['Model B contains more'].append(hypotheses['Model B contains more'])
            all_logs.append(logs)
            all_questions.append(questions)
        return all_hypotheses, all_logs, all_questions

class DualSidedLLMProposerNoQuestion(DualSidedLLMProposer):
