...
# Load your dataframe
results = pd.read_csv("./results_oz.csv")
...
def update_ui(cluster):
    ...
    score = cluster_results['score'].mean()
    nonzero_score = cluster_results[cluster_results['score'] != 0]['score'].mean()
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    summary_clusters = results.groupby(['parent_axis_name', 'parent_low', 'parent_high']).agg({'score': 'mean', 'question': 'count'}).reset_index()
    gr.Markdown(get_total_summary_string(summary_clusters))
    gr.DataFrame(summary_clusters[['parent_axis_name', 'parent_low', 'parent_high', 'question', 'score']], label="Summary of Axes")
